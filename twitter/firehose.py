...
class FilterPredicateMatcher(Thread):
    ...
    def __init__(self, mq_host, predicates, drop_dict):
        ...
        try:
            # Indpendent MQ connection for this thread
            ...
            self._channel.queue_declare(
                queue=Worker.DROPLET_QUEUE, durable=True)
    ...
    def run(self):
        ...
        if len(river_ids) > 0:
            ...
            self._channel.basic_publish(
                exchange='', routing_key=Worker.DROPLET_QUEUE,
...
class TwitterFirehoseWorker(Worker):
    ...
    def __get_filter_predicates(self, predicates):
        ...
        follow = (predicates.get('follow').keys()
                  if 'follow' predicates else None)
...
class FirehoseStreamListener(StreamListener):
    ...
    def __init__(
        self, mq_host, predicates, firehose_worker=None, predicate_type=None):
    ...
    def on_data(self, data):
       ...
       if 'in_reply_to_status_id' in data:
           ...
           FilterPredicateMatcher(self.mq_host, self.__predicate_list,
                                  drop_dict).start()
