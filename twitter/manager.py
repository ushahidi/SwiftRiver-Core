...
class TwitterFirehoseManager:
    ...
    def add_filter_predicate(self, mq, payload):
        ...
        if len(publish_data) > 0:
            ...
            # Construct the message to sent out to the process consuming
            # the firehose
        ...
        while True:
            try:
                if self.__predicates_changed:
                    ...
                    channel.queue_declare(queue=utils.FIREHOSE_QUEUE,
                                          durable=False)
                    channel.basic_publish(exchange='',
                                          routing_key=utils.FIREHOSE_QUEUE,
                                          body=message)
                ...
                log.error("%s Firehose manager error connecting to the MQ, retrying" % msg)
                time.sleep(60)
    ...
    def handle_mq_response(self, ch, method, properties, body):
        try:
            ...
            if method.routing_key == "web.channel_option.twitter.delete":
                ...
                self.__firehose_manager.delete_filter_predicate(
                    self.mq, payload)
