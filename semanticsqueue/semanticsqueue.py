...
class SemanticsQueueWorker(Worker):
    ...
    def handle_mq_response(self, ch, method, properties, body):
        ...
        log.info(" %s droplet received with id %d" % (self.name,
            droplet.get('id', 0)))
        ...
        while not resp:
            try:
                resp, content = self.h.request(
                    api_url, 'POST', body=urlencode(post_data),
                    headers=headers)
        ...
        if content:
            ...
            if response.get('status') == 'OK':
                ...
                # Each tag is stored in the format:
                # {tag_name: 'tag', 'tag_type': tag_type}
                droplet['tags'] = []
                ...
                log.debug('%s droplet meta = %r, %r' %
                    (self.name, droplet.get('tags'), droplet.get('places')))
