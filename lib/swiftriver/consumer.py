...
class Consumer(Thread):
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                channel.queue_declare(queue=self.queue_name, durable=self.durable_queue)
                ...
                channel.basic_consume(self.handle_message, queue=self.queue_name)
    ...
    def handle_message(self, ch, method, properties, body):
        self.pending += 1
        self.message_queue.put((method.routing_key, method.delivery_tag, body), False)
        ...
        while not self.confirm_queue.empty() or self.pending == self.prefetch_count:
