...
If the scheduler requests a url from the cache, the fetcher will return droplets form the 
...
class RssFetcherWorker(Worker):
    ...
    def __init__(self, name, job_queue, confirm_queue, db_config, drop_publisher, response_publisher):
    ...
    def work(self):
        ...
        if message.get('use_cache', False):
            ...
            if len(results):
                cache_found = True;
                log.info(" %s Served %s from the cache" % (self.name, message['url']))
        ...
        if not (message.get('use_cache', False) and cache_found):
            ...
            if message['last_fetch_etag']:
                d = feedparser.parse(message['url'], etag=message['last_fetch_etag'])
            elif message['last_fetch_modified']:
                d = feedparser.parse(message['url'], modified=message['last_fetch_modified'])
            else:
            ...
            self.response_publisher.publish({
                ...
                'last_fetch_modified': d.get('modified', message['last_fetch_modified']),
        ...
        if len(drops):
            ...
            c.executemany(
                "insert into `rss_cache` (`url`, `date_pub`, `drop`) values (%s, %s, %s)", drops[:50]
            )            
            c.close()
            self.db.commit()
    ...
    def run(self):
        try:            
            consumer = Consumer("rss-fetcher-consumer", self.mq_host, 
                                     'RSS_FETCH_QUEUE', {'prefetch_count': self.num_workers})
            ...
            for x in range(self.num_workers):
                RssFetcherWorker("worker-" + str(x), consumer.message_queue, 
                                 consumer.confirm_queue, self.db_config, drop_publisher, response_publisher)
