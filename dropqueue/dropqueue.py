...
# encoding: utf-8
"""
Creates drops in the SwiftRiver db by posting drops published to the DROPLET_QUEUE queue on the 
MQ to the api endpoint /api/drops and publishes newly created drops back to the metadata exchange
...
class DropQueueWorker(Worker):
    ...
    def __init__(self, name, job_queue, confirm_queue, meta_publisher, api_url):
    ...
    def post_drops(self):
        ...
        while not resp:
            try:
                resp, content = self.h.request(self.api_url, 'POST', body=json.dumps(drops))
...
class DropQueueDaemon(Daemon):
    ...
    def __init__(self, num_workers, batch_size, mq_host, api_url, pid_file, out_file):
    ...
    def run(self):
        ...
        drop_consumer = Consumer("dropqueue-consumer", self.mq_host, 
                                 Consumer.DROPLET_QUEUE, 
                                 {'durable_queue': True, 'prefetch_count': self.num_workers * self.batch_size})
        
        for x in range(self.num_workers):
            DropQueueWorker("dropqueue-worker-" + str(x), drop_consumer.message_queue, 
...
if __name__ == "__main__":
    ...
    try:
        ...
        daemon = DropQueueDaemon(num_workers, batch_size, mq_host, api_url, pid_file, out_file)
        if len(sys.argv) == 2:
            if 'start' == sys.argv[1]:
                daemon.start()
